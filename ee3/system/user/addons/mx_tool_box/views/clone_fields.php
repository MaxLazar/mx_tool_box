...
<?php //success
echo form_open(ee('CP/URL')->make('cp/addons/settings/mx_tool_box/clone_index'));
?>		
...
<tbody>
		...
		<?php
			...
			//
					...
					$out .= '<tr  id="field_'.$field->field_id.'" rel="'.$field->field_id.'"><td>'.$field->field_id.'</td><td class="label">'.$field->field_label.'</td><td class="name">'. $field->field_name.'</td><td></td><td><a class="clone_btn"><img src="'.$img_path.'user/mx_tool_box/images/clipboard_copy.png" alt="'.lang('2clone_line').'" title="'.lang('2clone_line').'" /></a></td><td><input type="checkbox" name="clone_'.$field->field_id.'" value=""></td></tr>';
					
					if (isset($errors[$field->field_id])){
						foreach ($errors[$field->field_id]  as $error_line)
						{
							$out .= '<tr id="row_'.$error_line['id'].'"><td  style="width:30px"><input name="clone[field_order][]"  value="'.$error_line['id'].'" type="hidden"/><input name="clone[copy_'.$error_line['id'].']"  value="'.$field->field_id.'" type="hidden"/><a OnClick="delete_line(\'row_'.$error_line['id'].'\');"><img src="'.$img_path.'user/mx_tool_box/images/cancel.png" alt="'.lang('remove_line').'" title="'.lang('remove_line').'" /></a></td><td><input dir="ltr" style="width: 100%;" name="clone[label_'.$error_line['id'].']" id="" value="'.$error_line['label'].'" size="20" maxlength="120" class="input"type="text"></td><td><input dir="ltr" style="width: 100%;" name="clone[name_'.$error_line['id'].']" id="" value="'.$error_line['name'].'" size="20" maxlength="120" class="input" type="text"></td><td></td><td style="width:30px;"><img src="'.$img_path.'user/mx_tool_box/images/hand_1.png" alt="'.lang('field_name_error').'" title="'.lang('field_name_error').'" /></td><td></td></tr>';
						}
					}
				}
				echo $out;
...
<script type="text/javascript">
	jQuery(public function() {
		...
		jQuery(".clone_btn").click(public function () {	
			...
			template = '<tr id="row_' + id + '"><td  style="width:30px"><input name="clone[field_order][]"  value="' + id + '" type="hidden"/><input name="clone[copy_' + id+ ']"  value="' + _tr.attr("rel") + '" type="hidden"/><a OnClick="delete_line(\'row_' + id + '\');"><img src="<?php echo($img_path);?>user/mx_tool_box/images/cancel.png" alt="<?=lang('remove_line')?>"  title="<?=lang('remove_line')?>" /></a></td><td><input dir="ltr" style="width: 100%;" name="clone[label_'+id+']" id="" value="' + field_label + '" size="20" maxlength="120" class="input"type="text"></td><td><input dir="ltr" style="width: 100%;" name="clone[name_'+id+']" id="" value="'+ field_name + '" size="20" maxlength="120" class="input" type="text"></td><td></td><td style="width:30px;"></td><td></td></tr>';
	...
	public function delete_line (_tr){
